...
#### File Naming
- **Pages:** `[Name]Page.razor` (e.g., `SettingsPage.razor`)
- **Components:** `[Purpose][Type].razor` (e.g., `ToggleRow.razor`)
- **Models:** `[Entity].cs` (e.g., `UserSettings.cs`)
- **Services:** `[Purpose]Service.cs` (e.g., `ThemeService.cs`)

#### CSS Scoping
- **Global styles:** `wwwroot/app.css`
- **Theme tokens:** `wwwroot/theme.css`
- **Component styles:** `[ComponentName].razor.css` (scoped automatically)
...
{
    SettingsState.Current.Theme = ThemeMode.Dark;
    StateHasChanged();
}
```
...
{
    await SettingsState.UpdateAsync(
        s => s.Theme = ThemeMode.Dark,
        "Theme",
        "Light",
        "Dark"
    );
...
/* Automatically disabled when reduce-motion is true */
```
...
**1. Update the Model** (`UserSettings.cs`):
```csharp
...
**2. Add UI Control** (`SettingsPage.razor`):
```razor
<ToggleRow 
    Label="My New Setting" 
    Description="What this setting does"
    Value="settings.MyNewSetting" 
    ValueChanged="async (value) => await OnMySettingChanged(value)" />
```

**3. Add Handler** (`SettingsPage.razor`):
```csharp
private async Task OnMySettingChanged(bool value)
{
    await SettingsState.UpdateAsync(
        s => s.MyNewSetting = value,
        "My Setting",
        settings.MyNewSetting ? "On" : "Off",
        value ? "On" : "Off"
    );
}
```

**4. Apply to UI** (if needed in `ThemeService.cs`):
```csharp
public async Task ApplyThemeAsync(UserSettings settings)
{
    ...
    await _jsRuntime.InvokeVoidAsync(
        "eval",
        $"document.documentElement.setAttribute('data-my-setting', '{settings.MyNewSetting}')"
    );
}
```
...
**5. Add CSS** (`theme.css`):
```css
[data-my-setting="true"] {
    /* Styles when setting is enabled */
}
```

### Adding a New Preview Component

**1. Create Component** (`Components/Views/PreviewCards/MyCard.razor`):
```razor
@* Copyright Slav Povstianoj 2026 *@

@namespace SettingsPlayground.Components.Views.PreviewCards
...
**2. Create Styles** (`MyCard.razor.css`):
```css
.my-card {
...
**3. Add to Preview Page** (`PreviewPage.razor`):
```razor
...
### Adding a New Reusable Control

Follow the pattern from existing rows (`ToggleRow`, `SliderRow`, etc.):
...
@code {
    ...
    [Parameter] public YourType Value { get; set; }
    [Parameter] public EventCallback<YourType> ValueChanged { get; set; }
}
```
...
**Symptom:** Changes reset when app restarts

**Solution:**
1. Check file permissions in app data directory
2. Verify `SettingsStore.SaveAsync()` is called
3. Check for exceptions in debug output

### Theme Not Applying

**Symptom:** UI doesn't change when adjusting theme settings

**Solution:**
1. Open browser dev tools (if debugging)
2. Check console for JavaScript errors
3. Verify `settingsPlayground.applyTheme` is defined
4. Check HTML element has `data-theme` attribute
5. Verify CSS custom properties are loaded

### Undo Not Working

**Symptom:** Undo button appears but doesn't revert changes

**Solution:**
1. Verify `settingName`, `oldValue`, `newValue` are passed to `UpdateAsync()`
2. Check history stack has items (`CanUndo` property)
3. Ensure `UndoAsync()` is being called

### Import Fails

**Symptom:** JSON import shows error message

**Common causes:**
- Invalid JSON syntax → Use JSON validator
- Unknown enum values → Check spelling matches exactly
- Missing required fields → Ensure all properties present
- Wrong schema version → App may not support older/newer versions

**Solution:** Export current settings to see proper format, then modify

### Performance Issues

**Symptom:** App feels slow when changing settings

**Solution:**
1. Enable "Reduce Motion" to disable transitions
2. Check for multiple event subscriptions (memory leak)
3. Verify components properly implement `IDisposable`
...
1. **Rapid Testing:** Use Import/Export to quickly switch between setting configurations
2. **Sharing Themes:** Export your favorite settings and share JSON with others
3. **Development Mode:** Keep Preview and Settings side-by-side (desktop only)
4. **Keyboard Navigation:** All controls support keyboard/screen readers

### For Developers

1. **Hot Reload:** Most Razor changes apply without rebuild (MAUI hot reload)
2. **Browser DevTools:** Debug JavaScript/CSS using Chrome DevTools (Android) or Safari (iOS)
3. **Design Token Playground:** Use browser console to test token changes:
   ```javascript
   document.documentElement.style.setProperty('--accent', '#ff0000');
   ```
4. **State Inspection:** Add breakpoints in `SettingsState.UpdateAsync()` to debug flow

---

## License & Credits

**Copyright © 2026 Slav Povstianoj**
